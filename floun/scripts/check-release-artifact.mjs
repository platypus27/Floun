...
const aiKeyPattern =
  /(?<![0-9A-Za-z_-])(?:AIza[0-9A-Za-z_-]{20,}|sk-[0-9A-Za-z_-]{20,})/;
...
function assertZipEntryFileTypesAreExpected(entries) {
  for (const entry of entries) {
    ...
    if (!allowedEntryExtensions.has(extname(entry).toLowerCase())) {
      throw new Error(
        `Release artifact contains unexpected file type: ${entry}`,
      );
    }
  }
}
...
function zipReference(reference) {
  ...
  if (/^(?:https?|data|chrome|mailto):/i.test(path)) {
    throw new Error(
      `Release artifact contains forbidden external or data reference: ${reference}`,
    );
  }
  const normalized = path.replace(/^\/+/, "").replace(/^(?:\.\/)+/, "");
  if (normalized.startsWith("../")) {
    throw new Error(
      `Release artifact contains unsupported parent-relative reference: ${reference}`,
    );
  }
  return normalized;
}
...
function assertReference(label, reference, entries) {
  ...
  if (normalized && !entries.has(normalized)) {
    throw new Error(
      `${label} references missing release artifact entry: ${reference}`,
    );
  }
}
...
function assertPackagedManifestKeysAreExpected(manifest) {
  ...
  assertObjectKeys(
    "Packaged manifest background",
    manifest.background,
    expectedManifestBackgroundKeys,
    "Packaged manifest background contains unexpected key",
  );
  assertObjectKeys(
    "Packaged manifest action",
    manifest.action,
    expectedManifestActionKeys,
    "Packaged manifest action contains unexpected key",
  );
  assertObjectKeys(
    "Packaged manifest content_security_policy",
    manifest.content_security_policy,
    expectedManifestCspKeys,
    "Packaged manifest content_security_policy contains unexpected key",
  );
  assertObjectKeys(
    "Packaged manifest icons",
    manifest.icons,
    expectedManifestIconKeys,
    "Packaged manifest icons contains unexpected key",
  );
  assertObjectKeys(
    "Packaged manifest action default_icon",
    manifest.action.default_icon,
    expectedManifestIconKeys,
    "Packaged manifest action default_icon contains unexpected key",
  );
}

export function assertQaEvidenceMatchesArtifact(
  evidencePath,
  canonical,
  alias,
) {
  ...
  const requirements = [
    [
      ...
      `- Package path: \`floun/release/floun-${version}.zip\``,
    ],
    [
      "alias package path",
      `- Alias package path: \`floun/release/floun-${aliasVersion}.zip\``,
    ],
    ["canonical SHA-256", `- SHA-256: \`${canonical.hash}\``],
    ["alias SHA-256", `- Alias SHA-256: \`${alias.hash}\``],
    ["artifact size", `- Size bytes: \`${canonical.size}\``],
    [
      "deterministic packaging SHA-256",
      `produced matching SHA-256: \`${canonical.hash}\``,
    ],
  ];
  ...
  const entrySection = content.match(
    /Required archive entries:\s*(?<entries>(?:\r?\n- `[^`]+`)+)/s,
  );
  if (!entrySection?.groups?.entries) {
    throw new Error(
      "QA evidence mismatch: required archive entries section is missing or malformed.",
    );
  }
  const evidenceEntries = [
    ...entrySection.groups.entries.matchAll(/- `(?<entry>[^`]+)`/g),
  ].map((match) => match.groups.entry);
  assertStringSet(
    "QA evidence archive entries",
    evidenceEntries,
    canonical.entries,
  );
}
...
export function validateReleaseArtifact({
  ...
  requiredEntries.forEach((entry) => {
    if (!entries.has(entry))
      throw new Error(`Release artifact is missing required entry: ${entry}`);
  });
  requiredPrefixes.forEach((prefix) => {
    if (![...entries].some((entry) => entry.startsWith(prefix))) {
      throw new Error(
        `Release artifact is missing required directory: ${prefix}`,
      );
    }
  });
  for (const entry of entries) {
  ...
  const thirdPartyNotices = strFromU8(archive["THIRD_PARTY_NOTICES.txt"]);
  ...
  if (manifest.manifest_version !== 3)
    throw new Error("Packaged manifest must use manifest_version 3.");
  if (manifest.version !== expectedVersion) {
    throw new Error(
      `Packaged manifest version ${manifest.version} does not match package version ${expectedVersion}.`,
    );
  }
  assertStringSet(
    "Packaged manifest permissions",
    manifest.permissions,
    expectedPermissions,
  );
  assertStringSet(
    "Packaged manifest host_permissions",
    manifest.host_permissions,
    expectedHostPermissions,
  );
  if (manifest.content_scripts !== undefined)
    throw new Error(
      "Packaged manifest must not include always-on content_scripts.",
    );
  if (manifest.background.service_worker !== "background.js")
    throw new Error(
      "Packaged manifest background service worker must be background.js.",
    );
  if (manifest.background.type !== "module")
    throw new Error("Packaged manifest background worker must be a module.");
  if (
    manifest.content_security_policy.extension_pages !==
    expectedExtensionPagesCsp
  ) {
    throw new Error(
      `Packaged manifest extension_pages CSP must be ${expectedExtensionPagesCsp}.`,
    );
  }

  assertReference(
    "Packaged manifest default_popup",
    manifest.action.default_popup,
    entries,
  );
  assertReference(
    "Packaged manifest background service_worker",
    manifest.background.service_worker,
    entries,
  );
  Object.values(manifest.icons).forEach((reference) =>
    assertReference("Packaged manifest icons", reference, entries),
  );
  Object.values(manifest.action.default_icon).forEach((reference) =>
    assertReference(
      "Packaged manifest action default_icon",
      reference,
      entries,
  ...
  if (/<script\b(?![^>]*\bsrc\s*=)[^>]*>/is.test(indexHtml))
    throw new Error("Packaged index.html must not contain inline scripts.");
  if (/\s+on[a-z]+\s*=/i.test(indexHtml))
    throw new Error(
      "Packaged index.html must not contain inline event handlers.",
    );
  [...indexHtml.matchAll(/(?:src|href)=["'](?<path>[^"']+)["']/g)].forEach(
    (match) =>
      assertReference("Packaged index.html", match.groups.path, entries),
  );

  const backgroundJs = strFromU8(archive["background.js"]);
  [...backgroundJs.matchAll(/["'](?<path>\.\/assets\/[^"']+)["']/g)].forEach(
    (match) =>
      assertReference("Packaged background.js", match.groups.path, entries),
  );
  ...
  for (const [entry, data] of Object.entries(archive)) {
    ...
    if (forbiddenText.some((value) => content.includes(value))) {
      throw new Error(
        `Release artifact contains forbidden fixture or secret marker in ${entry}`,
      );
    }
    if (
      /(?:(?:\/\/)[#@]\s*sourceMappingURL\s*=|\/\*[#@]\s*sourceMappingURL\s*=)/im.test(
        content,
      )
    ) {
      throw new Error(
        `Release artifact contains forbidden source map reference in ${entry}`,
      );
    }
    if (
      extension === ".css" &&
      /(?:@import\s+(?:url\()?\s*["']?\s*(?:(?:https?|data|chrome|mailto):|\/\/)|url\(\s*["']?\s*(?:(?:https?|data|chrome|mailto):|\/\/))/i.test(
        content,
      )
    ) {
      throw new Error(
        `Release artifact contains forbidden external CSS reference in ${entry}`,
      );
    }
    if (containsAiApiKey(content)) {
      throw new Error(
        `Release artifact contains an AI API-key-like value in ${entry}`,
      );
    }
  }
...
function main() {
  ...
  const evidencePath =
    argumentValue("--qa-evidence") ||
    join(repoRoot, "docs", "release", version, "QA_EVIDENCE.md");
  const canonical = validateReleaseArtifact({ zipPath: canonicalPath });
  const alias =
    aliasPath === canonicalPath
      ? canonical
      : validateReleaseArtifact({ zipPath: aliasPath });

  if (alias.hash !== canonical.hash)
    throw new Error(
      `Release alias hash does not match canonical artifact: ${alias.path}`,
    );
